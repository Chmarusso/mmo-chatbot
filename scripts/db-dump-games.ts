...
const BACKUP_FILE = `${BACKUP_DIR}/games_${TIMESTAMP}.sql`;
const COMPRESSED_FILE = `${BACKUP_FILE}.gz`;

// Game-related tables to backup
const GAME_TABLES = [
  'GameCategory',
  'Game',
  'GameRating',
...
async function listRecentBackups() {
  try {
    const files = readdirSync(BACKUP_DIR)
      .filter(f => f.startsWith('games_') && (f.endsWith('.sql') || f.endsWith('.sql.gz')))
      .map(f => ({
        name: f,
    ...
    if (files.length > 0) {
      console.log('\n\x1b[33mRecent game backups:\x1b[0m');
      files.forEach(f => {
...
async function createBackup() {
  ...
  console.log('\x1b[33mStarting games database backup...\x1b[0m');
  console.log(`Database: ${dbConfig.database}`);
  console.log(`Host: ${dbConfig.host}:${dbConfig.port}`);
  console.log(`User: ${dbConfig.user}`);
  console.log('');
  console.log('\x1b[33mTables to backup:\x1b[0m');
  GAME_TABLES.forEach(table => {
    console.log(`  - ${table}`);
  });
  console.log('');
  ...
  // Add each table (quoted for case-sensitive PostgreSQL tables)
  GAME_TABLES.forEach(table => {
