...
export default async function DashboardPage() {
  ...
  if (hasPreferences) {
    const preferenceWhere: Prisma.ProfileWhereInput = {
      ...(profile.gamePref ? { gamePref: { equals: profile.gamePref as any } } : {}),
      ...(profile.language ? { language: { equals: profile.language as any } } : {}),
      ...(profile.playstyle ? { playstyle: { equals: profile.playstyle as any } } : {}),
      ...(profile.timeSlot ? { timeSlot: { equals: profile.timeSlot as any } } : {}),
      NOT: { id: profile.id },
    };
      ...
      <main className="flex-1 space-y-8 px-4 py-6 pb-24 sm:px-6 lg:mx-auto lg:max-w-6xl lg:space-y-10 lg:px-12 lg:py-12 lg:pb-12">
        <header className="space-y-2 text-center lg:text-left">
          ...
          <p className="text-sm text-gray-400 lg:max-w-2xl lg:text-base">
            Swipe to squad up with players who share your MMO, time slot, language, and playstyle.
          </p>
        </header>

        {!hasPreferences ? (
          <div className="mx-auto max-w-md rounded-3xl border border-accent-cyan/30 bg-surface/70 p-8 text-center shadow-glow lg:mx-0 lg:max-w-xl lg:text-left">
            ...
            <p className="mt-2 text-sm text-gray-300 lg:text-base">
              Tell us your preferred game, time slot, language, and playstyle. That’s how we deliver top-tier matches.
            </p>
            <Link
              href="/profile"
